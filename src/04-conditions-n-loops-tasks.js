...
function doRectanglesOverlap(rect1, rect2) {
  ...
  if (rect2Right < rect1.left) return false;
  if (rect1Bottom < rect2.top) return false;
  if (rect2Bottom < rect1.top) return false;
  return true;
}
...
function isInsideCircle(circle, point) {
  ...
  if ((circleX - point.x) ** 2 + (circleY - point.y) ** 2 < circle.radius ** 2) return true;
  return false;
}
...
function findFirstSingleChar(str) {
  ...
  const result = arr.filter((v, i, s) => s.filter((it) => it === v).length === 1);
